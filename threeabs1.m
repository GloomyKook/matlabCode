clc
clear
Dir=dir('C:\Users\zhuan\Desktop\10\*.BMP');
[m1,n1]=size(Dir);
I=imread(['C:\Users\zhuan\Desktop\10\',num2str(m1),'.bmp']);
[m,n]=size(I);
%n=n/3;
height=m;
width=n;
...
for i=1:3
    for j=1:4
        C{i,j} = zeros(height,width);%生成3行4列的每一个元素都是0矩阵
    end
end

%把图片的像素信息导入到C中
for s=1:12
    I=imread(['C:\Users\zhuan\Desktop\10\',num2str(s),'.BMP']);
    if(s<=4)
          C{1,s} = I;
    end
    if(s>4&&s<=8)
